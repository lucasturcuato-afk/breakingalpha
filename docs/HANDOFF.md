...
**Date:** 2026-05-02 morning (PT)
**Last session focus:** Wave 3 web-search fallback merged (#176); truncation fix on web-fallback memos (commit 7e07688); follow-up items surfaced (typo propagation, citation parity). Wave 2 PRs: #171 merged (avatar), #175 open (email polish), #172/#173/#174 draft (audits). OAuth + SITE_URL config still pending.
**Status:** PR #176 merged; web_search_cache.sql DDL pending manual Supabase apply before flag enable. Follow-up work: typo normalization for web-fallback queries, citation parity for article-grounded memos.
...
- **Follow-up work — Wave 3 post-merge items (not blocking)**
  - **(a) Typo propagation in web-fallback memos** — user's raw misspelled query (e.g. "Anthropc") flows into memo title and system prompt; Gemini echoes typo throughout body. Fix: canonicalization pass between query string and memo prompt. Surfaces: `src/app/api/companies/web-fallback/route.ts` (where canonicalName computed), `src/app/company/page.tsx` (where passed to MemoModal). Options: extract from top web results, use existing `canonicalize()` map (src/lib/company-intel.ts:255-297), or LLM pre-pass.
  - **(b) Citation parity for article-grounded memos** — PR #176 added per-claim `[n]` citations to web-fallback (every fact ends with citation marker, renders numbered source list). Article-grounded memos lack this (buildMemoSystemPrompt doesn't require, MemoModal doesn't render). Fix: extend buildMemoSystemPrompt to require citations; extend MemoModal to accept optional sources prop for article path; pass article URLs from /api/memo article-grounded branch. Surfaces: `src/lib/company-intel.ts:737-793` (prompt), `src/components/memo/MemoModal.tsx`, `src/app/api/memo/route.ts` (article branch).
  - **(c) Unspecified third follow-up** — Noah mentioned "three" follow-ups but detailed only two. Capture in next session.

**Open Wave 2 PRs (ready or draft)**
- **PR #175 — fix(email): morning brief polish + view-in-browser, issue numbering, unsubscribe** — branch w2/email-polish, OPEN ready-for-review. New `sql/brief_email_unsubscribe.sql` DDL (adds `briefings.issue_number int`, `user_profiles.brief_email_subscribed bool`; idempotent). Email helpers: site-url resolver, issue-number caching (MAX(issue_number)+1 strategy, soft-fail if column missing), HMAC-sha256 unsubscribe tokens. New `/api/unsubscribe` GET/POST route (RFC 8058 one-click). Updated brief email template (View-in-browser bar, Issue #N line, footer Unsubscribe link). Updated send-email route (per-recipient loop, List-Unsubscribe headers, filterUnsubscribed() opt-out, ensureIssueNumber() caching). Schema deviation noted: spec called for sent_at + COUNT(*) numbering, but `briefings` has no sent_at column; used MAX(issue_number)+1 instead. **MANUAL STEP REQUIRED:** Apply `sql/brief_email_unsubscribe.sql` to prod Supabase before merging.
...
**Local worktrees — ready for cleanup:** `/Users/noahhanning/ba-w3-webfallback` (PR #176, now merged). Plus earlier merged ones: `/Users/noahhanning/ba-w2-email` (PR #175 not yet merged), `/Users/noahhanning/ba-w2-avatars` (PR #171 merged), `/Users/noahhanning/ba-w2-entityaudit` (PR #173 draft), `/Users/noahhanning/ba-w2-evidenceaudit` (PR #172 draft), `/Users/noahhanning/ba-w2-cintelaudit` (PR #174 draft). Plus three Wave 1: `/Users/noahhanning/ba-w1-mood`, `/Users/noahhanning/ba-w1-trackrec`, `/Users/noahhanning/ba-w1-briefload` (all branches merged). **Total: 9 worktrees pending cleanup.**
...
**Pending DDL and migrations (2026-05-02)**
...
- **sql/brief_email_unsubscribe.sql** — Email feature (PR #175, open) requires manual Supabase DDL apply before merge: adds `briefings.issue_number int`, `user_profiles.brief_email_subscribed bool default true`. Idempotent. **REQUIRED before merging PR #175.**
