...
### Ruling 25: Ask is directory-first, and the answer screen retires
...
What C does. The field moves to the top and filters the fifty rows the server
has already read. The company directory becomes the screen rather than a section
below three destination rows. Sector demotes to an inline tail after the name so
the row loses its second line. The three destinations demote to 46px rows
underneath, each carrying a real figure with its window spelled beside it.
Nothing is pinned to the bottom except the tab bar, and that is what buys the
room: the scroll window goes from 497px to roughly 630px, so six companies and
all three destinations clear the fold where today none of the six does.

`/ask?q=` becomes a state of this screen rather than a second screen. That is
the half of the ruling with the most consequence, because it settles three
separate defects at once instead of repairing them one at a time.

Why this satisfies Ruling 20 better than fixing the answer screen would. Nothing
links to `/ask?q=`, so the RSC prefetch cannot reach it, and the filter runs on
the client over rows already in the payload, so nothing is server-rendered off
`?q=` and no model is called on a path the framework can walk. Ruling 20 is
preserved by construction rather than by a `prefetch={false}` that does not hold.

What was declined, and these are not oversights. `mention_count` as a per-row
figure, because a number beside each name invites reading the column as a
ranking of importance when it is a count of articles. `key_themes`, because the
arrays run 25 to 97 items with nothing ranking them, so any first-N is
arbitrary. And the send button, because a filter has no submit moment and a
submit control would promise a navigation that does not happen.
...
labelled for filtering may still be confused. The owner's instruction was to
watch for it in testing rather than pre-solve it, so no mode toggle, segmented
control or intent-detection affordance was built.

This ruling follows Ruling 23, which authorised the redraw, and Ruling 24, which
kept Company Intel under the Ask pole and required a real directory inside it. C
is that directory. There is no separate Company Intel artboard because the
