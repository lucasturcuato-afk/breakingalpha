...
Return a single JSON object matching this shape and nothing else. No prose outside the JSON object. No markdown code fences. No commentary.
{
  ...
  "lead":    { "paragraphs": string[] (the "What Just Changed" body in developments-led mode, or the "Coverage Note" in context-led mode; one element per development paragraph) },
  "context": { "paragraphs": string[] (the "Cross-Signals" body, one element per analytical paragraph) },
  "watch":   { "items": [ { "thesis_path": "bull" | "bear", "description": string, "probability": "low" | "medium" | "high" } ] },
  "sources": [ { "n": integer, "name": string, "url": string, "type"?: "primary" | "tier-1" | "web" } ]
}
Every [n] citation marker in tldr and inside lead.paragraphs / context.paragraphs / watch.items[].description must map to an entry in sources[].n. Source name should be the publisher (e.g., "Bloomberg", "Reuters", "Wall Street Journal"). Source url should be the article URL when present in the input pool. type is optional and may be omitted. If you cannot determine a URL for a source, omit that source entry rather than inventing one.
...
lead.paragraphs (What Just Changed):
Draw exclusively from COMPANY DEVELOPMENT ARTICLES. For each development, apply this filter: does it involve a specific dollar figure, a named strategic counterparty, a named product with a deployment status, or a direct change to capital structure or competitive position? If yes, cover with two-sentence discipline. If no, omit. Cap at the 3-5 highest-density developments. Emit one element of lead.paragraphs per development; do NOT collapse multiple developments into a single string. name deals, amounts, counterparties, dates where available. No vague statements.
Two-sentence discipline per development paragraph:
...
context.paragraphs (Cross-Signals):
Draw exclusively from SECTOR CONTEXT ARTICLES. Emit 2-4 paragraphs (one element of context.paragraphs each). Across the paragraphs: connect this company's developments to the most relevant sector-level or competitor-level moves in the article pool, name specific peers/competitors/macro forces, and state whether sector momentum supports or undermines the company's current trajectory. The final paragraph must end with a binary directional verdict using: "Sector momentum [supports / does not support / is net negative for / is net positive for] ${companyName}'s [specific named aspect of its business] in the [specific timeframe]." A verdict containing "mixed", "presents", "both", or "while" is a hedge, rewrite until one direction is stated without qualification. Cite [n] for each named peer or specific figure. (catalysts, risks, competitive dynamics, valuation considerations, macro tailwinds/headwinds)

watch.items (What To Do With This):
Emit 2-3 items. Each item is a single object with fields {thesis_path, description, probability}. thesis_path is "bull" or "bear" -- typically one bull plus one bear; a third item is permitted when a second adverse path is materially distinct. description is a single 50-75 word body that names the specific trigger, the thesis it confirms or weakens, and the recommended action. probability is one of "low" | "medium" | "high" reflecting how likely the trigger is to fire on the relevant horizon. Cite [n] inside description for each named figure or event. Do not write If/Then bullets inside the description; the structure is captured by thesis_path/probability already.
...
lead.paragraphs: Coverage Note. Emit a single paragraph (one element of lead.paragraphs) stating briefly that no direct company development articles cleared the filter in the current feed window, and that this brief draws from sector context only. One or two sentences.

context.paragraphs: Draw exclusively from SECTOR CONTEXT ARTICLES. This is the primary analytical section. Emit 3-4 paragraphs (one element of context.paragraphs each). Draw implications, name competitive dynamics, and connect sector moves to this company's specific situation. The final paragraph ends with the binary directional verdict in the same format as developments-led. Cite [n] for each named peer or specific figure.

watch.items: Emit 2-3 items with the same {thesis_path, description, probability} structure as developments-led. At least one item must name the specific catalyst that would change the signal quality from context-led to developments-led; encode that as a "bull" thesis_path with the description starting from the catalyst.
...
DENSITY TARGETS (non-negotiable floors -- this is the failure mode the schema must defend against):
- Total memo: 600-800 words across all paragraphs[] and watch.items[].description fields combined.
- lead.paragraphs[]: at least 3 elements, capped at 5. Each element MUST be at least 80 words.
- context.paragraphs[]: at least 3 elements, capped at 4. Each element MUST be at least 100 words.
- watch.items[]: at least 4 items total, with at least 2 thesis_path="bull" and at least 2 thesis_path="bear". Each items[].description MUST be at least 50 words.
- Every paragraph (lead AND context) MUST contain at least one specific dollar figure, percentage, named entity (company / product / executive / transaction), or temporal anchor. Generic sector framing without specifics fails this floor.

Every sentence must earn its place by containing a specific fact or a non-obvious implication -- if it contains neither, cut it. Filler is banned. Specificity is required. Below the minimum density floors stated above is a failure mode; verbosity without specificity is also a failure mode. Trim filler, never trim specifics. No bullet points outside "What To Do With This." No markdown headers beyond the bold section labels already specified. The em-dash character (U+2014) is banned everywhere in the memo output without exception, including inside bullet points. Use a period and start a new sentence instead. The hyphen-minus character is fine. Signal Quality: verbatim reproduction only, no commentary. Output only user-facing prose inside the JSON string fields, never reproduce bracketed instructions or meta-directives. The JSON object itself is the entire output. Do not wrap it in code fences. Do not emit any text before or after the closing brace.

Provenance is non-negotiable: every factual sentence inside lead.paragraphs, context.paragraphs, and watch.items[].description ends with at least one bracketed citation [n] mapping to the numbered entries in the sources[] array. The frontend renders the source list below the memo so the reader can click through. A memo without citations fails this requirement and must be rewritten.`;
}
