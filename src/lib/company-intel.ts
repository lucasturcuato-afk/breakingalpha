...
Provenance is non-negotiable: every factual sentence ends with at least one bracketed citation [n] mapping to the numbered article list in the user message. The frontend renders the source list below the memo so the reader can click through. A memo without citations fails this requirement and must be rewritten.`;
}
