...
function RecordHero({
  ...
  // `--cream` is a near-white in light and a near-black in dark, so in dark
  // this block drew its near-black ink on the near-black `--elevated` fill:
  // measured 1.15:1 against a 4.5 floor, every string in the block affected,
  // the ring track included since it is drawn with currentColor. Light
  // measures 17.19:1, which is why the block read as fine to anyone reviewing
  // in light. `--text-primary` is the dark theme's own ink and measures
  // 13.61:1 on the same fill. Values are in src/styles/tokens.css rather than
  // repeated here. The dial, the figure, the ring and this block's own
  // vocabulary are deliberately untouched.
  return (
