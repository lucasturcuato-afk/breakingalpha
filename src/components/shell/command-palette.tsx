...
const commands: CommandItem[] = [
  ...
  { id: "thesis", label: "Thesis Board", section: "Research", href: "/thesis-board", icon: <FileText size={14} /> },
          ...
          <div className="max-h-[320px] overflow-y-auto py-2">
            {filtered.length === 0 ? (
              ...
              Array.from(sections.entries()).map(([section, items]) => (
                <div key={section}>
                  <p className="px-4 pt-2 pb-1 font-sans text-[10px] font-semibold uppercase tracking-widest text-text-faint">
                    {section}
                  </p>
                  {items.map((item) => {
                    const idx = flatIndex++;
