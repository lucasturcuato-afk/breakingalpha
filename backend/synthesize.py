...
def run(brief_type="morning"):
    ...
    if brief_type == "evening":
        try:
            ...
            evening_brief_id = inserted.get("id")
            review = generate_morning_review_for_evening(date.today(), supabase)
            if review and evening_brief_id:
                supabase.table("briefings").update(
                    {"morning_review": review}
                ).eq("id", evening_brief_id).execute()
                print(f"  🔁 Attached morning_review to evening brief {evening_brief_id}")
            elif review and not evening_brief_id:
                print("  ⚠ Generated morning_review but no evening_brief_id returned from insert")
        except Exception as e:
